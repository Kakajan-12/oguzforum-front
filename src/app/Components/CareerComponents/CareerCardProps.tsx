        ...
        return (
          ...
          shadow-slate-400">
            <div className="flex justify-between w-full items-center  ">
              <p className={`lg:text-2xl md:text-xl sm:text-sm transition-all duration-150 text-mainBlue  text-sm  ${activeIndex === index ? 'font-bold' : 'font-normal'} `}>{tittle}</p>
              ...
              {activeIndex === index && (
                ...
                transition={{duration: 0.3}}
                 className="  overflow-hidden text-mainBlue  font-normal flex flex-col ">
                  {/* {items.texts.map((li, i) => {
                    return <li className={`md:px-10 px-2 text-[12px] lg:text-lg  md:text-sm ${i === 0 ? 'mt-5' : 'mt-2'}`}>{li.li}</li>;
                  })} */}
                  <p className={`md:px-10 px-2 text-[12px] lg:text-lg  md:text-sm ${ 'mt-5'}`}>{tittle}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
