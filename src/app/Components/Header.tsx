...
export default function Header() {
    ...
    const toggleAboutMenu = () => setAboutDropdownOpen(!isAboutDropdownOpen);
    const toggleMobileMenu = () => setMobileMenuOpen(!isMobileMenuOpen);
    const toggleLang = () => setLangOpen(!isLangOpen);
    const t = useTranslations('Header');

    return (
        <header className={`fixed w-full z-20 ${isScrolled ? 'bg-custom-gradient' : 'bg-gradient'} transition-all duration-300`}>
            <div className="container mx-auto px-2">
                <div className="flex items-center justify-between h-20 sm:h-28">
                    <Link href="/" className="text-white font-extrabold">
                        <Image width={100} height={10} className="lg:w-36 xl:w-44 w-32 h-full" alt="Logo" src="/logoOguzForm.png" />
                    </Link>
                        ...
                        <ul className="flex flex-col space-y-2 p-4 mt-20 w-2/3">
                            ...
                            <li className="absolute left-10">
                                <div className="flex items-center" onClick={toggleLang}>
                                    <div>{locale.toUpperCase()}</div>
                                    <svg className="-mt-1 ml-1" width="12" height="9" viewBox="0 0 12 9" fill="none">
                ...
                {isMobileMenuOpen && <div className="fixed bg-black opacity-50 w-full h-screen top-0 left-0 z-0"></div>}
            </div>
        </header>
    );
}
