...
export default function Header() {
  ...
  useEffect(() => {
    ...
    if (saved !== null) {
      ...
      window.scrollTo({ top: parseInt(saved), behavior: "auto" }); //или smooth
    }
    // Mobile menu state
  ...
  const switchLanguage = (newLocale: string) => {
    ...
    // router.push(newPath);
    sessionStorage.setItem("_scrollY", String(window.scrollY));
    sessionStorage.setItem("_mobileMenuOpen", "true");
    router.push(pathname, { locale: newLocale });
  };

  return (
    <header
      className={`fixed w-full z-20 transition-all duration-300 ${isScrolled || isLoading ? "main-background-color" : "bg-gradient"}`}
    >
