...
const MainEvents = () => {
  ...
  function formatDateRange(start: string, end: string) {
    ...
    if (s.getMonth() === e.getMonth() && s.getFullYear() === e.getFullYear()) {
      return `${s.getDate()}–${e.getDate()} ${s.toLocaleDateString("en-GB", {
        month: "long",
        year: "numeric",
      })}`;
    }
                      ...
                      >
                        ...
                        <div className="pt-2">
                          <Image
                            width={300}
                            height={300}
                            src={resolveMediaUrl(slide.logo)}
                            alt={`Slide ${title}`}
                            className="w-fit h-14"
                          />
                        </div>
                        <div className="text-black text-sm md:text-md lg:text-lg pt-4 h-20 line-clamp-2 flex-shrink-0">
                          <RichText
                            htmlContent={
                              title.length > 90
                                ? title.slice(0, 50) + "..."
                        ...
                        <div className="text-xs md:text-sm opacity-75 pt-4">
                          {formatDateRange(slide.date, slide.end_date)}
                          {/* {new Date(slide.date).toLocaleDateString("tm-TM")} */}
                        </div>
