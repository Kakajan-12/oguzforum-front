...
const STAT_LABELS: { key: "speakers" | "delegates" | "countries" | "companies" | "media"; label: string }[] = [
...
export default function ProjectDetailPage() {
  ...
  const organizers = (data.organizers ?? []).filter((o) => o.organizer_en || o.organizer_logo);
  const participants = (data.participants ?? []).filter((p) => p.participant_logo);
        ...
        <div className="mt-8 grid grid-cols-1 gap-8 lg:mt-10 lg:grid-cols-2 lg:gap-12">
          ...
          <div>
            <div className="flex items-start gap-5">
              {data.logo && (
                <div className="relative hidden h-16 w-16 shrink-0 overflow-hidden rounded sm:block">
                  <Image
            ...
            <RichText
              ...
              className="mt-5 text-[0.95rem] leading-relaxed text-gray-600"
            />
              ...
              {organizers.map((org, i) => {
                const logo =
                  typeof org.organizer_logo === "string" && org.organizer_logo
                    ...
                    : null;
                return (
                  <div key={org.id ?? i} className="flex w-40 flex-col items-center text-center">
                    {logo && (
                      <div className="relative h-20 w-full">
                        <Image src={logo} alt={org.organizer_en || ""} fill sizes="160px" className="object-contain" />
                      </div>
                    )}
                    {org.organizer_en && (
                      <p className="mt-4 text-sm text-gray-600">{org.organizer_en}</p>
                    )}
                  </div>
                );
              })}
